...
1. **FeignTraceInterceptor** - 添加链路追踪头（`X-Trace-Id`、`X-Span-Id`）
2. **InternalRequestInterceptor** - 添加签名头（`x-sign`、`x-nonce-str`、`x-timestamp`）

**职责分离**：
- 每个拦截器专注于自己的职责
- 互不干扰，各司其职
- 日志由 `SingleLineHttpLogger` 统一处理（在 OkHttp 层面）
